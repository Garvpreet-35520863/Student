...
vector<Student*> readStudents(const string& filename) {
    ...
    while (getline(infile, line)) {
        ...
        string type;
        getline(ss, type, ','); // Read first field (C or R)
        ...
        if (tolower(type[0]) == 'c') {
            ...
            string idStr;
            getline(ss, idStr, ','); id = stol(idStr); //extracted string is converted to required value type
            getline(ss, unitId, ',');

            string levelStr; getline(ss, levelStr, ','); level = stoi(levelStr);

            string m1Str, m2Str, m3Str, m4Str, mFinalStr;
            getline(ss, m1Str, ','); m1 = stof(m1Str);
            getline(ss, m2Str, ','); m2 = stof(m2Str);
            getline(ss, m3Str, ','); m3 = stof(m3Str);
            getline(ss, m4Str, ','); m4 = stof(m4Str);
            getline(ss, mFinalStr, ','); mFinal = stof(mFinalStr);
            ...
            getline(ss, first, ',');
            getline(ss, last, ',');
            string idStr; getline(ss, idStr, ','); id = stol(idStr);

            string propStr, dissStr;
            getline(ss, propStr, ','); proposal = stof(propStr);
            getline(ss, dissStr, ','); dissertation = stof(dissStr);
